...
class AccessControlMiddleware:
    ...
    def check_user_permissions(self, request, url_name):
        ...
        user_urls = [
            ...
            'message_list', 'message_create', 'message_update', 'message_delete',
            'unsent_messages',
            'mailing_list', 'mailing_create', 'mailing_update', 'mailing_delete', 'mailing_send',
            'attempt_list', 'attempt_details',
            'profile', 'user_update', 'change_password', 'logout'
        ]
        ...
        if user.role == 'lector' and url_name in user_urls:
            messages.error(request, '❌ Доступ запрещен. Лекторы не могут управлять рассылками.')
            return redirect('mailing:home')
